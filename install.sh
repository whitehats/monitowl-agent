...
curl --progress-bar -o "${BINDIR}/monitowl-agent" "${BINURL}/${BINNAME}"
...
if [ -n "$(init --version | grep "systemd")" ]; then
    echo "Detected systemd, downloading service file from \`${SYSTEMDURL}\` to \`${SYSTEMDDIR}\`."
    curl --progress-bar -o "${SYSTEMDDIR}" "${SYSTEMDURL}"
    ...
    curl --progress-bar -o "${SYSVDIR}" "${SYSVURL}"
    service monitowl-agent stop
    service monitowl-agent start
    update-rc.d monitowl-agent enable
fi
