...
@pytest.mark.parametrize(['ts', 'dt'],
                         [(1381363200000, dt(2013, 10, 10, 0, 0)),
                          ...
                          (0, dt(1970, 1, 1)),
                          (0, tz('CET').localize(dt(1970, 1, 1, 1))),
...
@pytest.mark.parametrize(['dt_utc', 'dt_tz'],
                         [(dt(1970, 1, 1), tz('CET').localize(dt(1970, 1, 1, 1))),
                          (dt(2013, 1, 1, 15), tz('CET').localize(dt(2013, 1, 1, 16))),
                          (dt(2013, 1, 1, 15), tz('Europe/Warsaw').localize(dt(2013, 1, 1, 16))),
                          (dt(2013, 8, 1, 15), tz('Europe/Warsaw').localize(dt(2013, 8, 1, 17)))])
def test_datetime_to_utc(dt_utc, dt_tz):
    assert dt_utc == datetime_to_utc(dt_tz)
