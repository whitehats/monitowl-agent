...
class TypeRegistrySerializationBase(object):
    ...
    * If :meth:`TypeRegistry.TimeSeries` also needs to be serialized you have
    ...
    def type_to_schema(self, type_):
        ...
        if issubclass(type_, GenericContainer):
            return self._wrap_TimeSeries(self.type_to_schema(type_.item_type))

        elif type_ in self._type_registry.primitives:
    ...
    def schema_to_type(self, schema):
        ...
        if self._unwrap_TimeSeries(schema):
            return self._type_registry.TimeSeries(
                self.schema_to_type(self._unwrap_TimeSeries(schema)))
    ...
    # CamelCase in method name.
    # pylint: disable=C0103
    @staticmethod
    def _wrap_TimeSeries(subtype_name):
        '''Handle nesting types in TimeSeries building schema.'''
        return 'TimeSeries({})'.format(subtype_name)

    @staticmethod
    def _unwrap_TimeSeries(schema):
    ...
    def _select_serializer(self, type_, method_name='serialize_{type_name}'):
        ...
        if type_ not in self._type_registry:
            ...
            name = method_name.format(type_name='TimeSeries')
        else:
